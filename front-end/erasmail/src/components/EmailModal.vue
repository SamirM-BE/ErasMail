<template>
    <div class="modal" :class="{ 'is-active': showModal }">
        ...
        <div class="modal-card">
            ...
            <div class="modal-card-body pt-0 has-background-primary-light">
                <section class="is-top">
                    <div class="duplicate-message has-background-danger-light has-text-danger-dark has-text-centered"
                        ...
                        <p v-if="numberOfDuplicate==1"> There is {{numberOfDuplicate}} potential duplicate attachment
                        </p>
                        <p v-else> There are {{numberOfDuplicate}} potential duplicate attachments</p>
                    </div>
                    <div class="box py-1 has-less-shadow">
                ...
                <EmailForm :class="{'mt-6': numberOfDuplicate == 0, 'mt-7': numberOfDuplicate !== 0}" :emails="emails"
                    :selectAll="selectAll" :attachmentStylesList="attachmentStyles" :reset="!showModal"
...
export default {
    ...
    watch: {
        selectedEmailsHasAttachment(newValue) {
            // if there is at least one attachment among the selected emails and the `selectedAction` is not to delete the attachments
            if (newValue && this.selectedAction !== this.removeAttachment) {
                this.selectedAction = 'Select an action'
            } 
            // if there are no attachments among the selected emails and there are selected emails then the only possibility is to delete the emails
            else if (!newValue &&  this.checkedEmails.length){
