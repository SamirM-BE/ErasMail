    ...
    if type(bodystructure_header[0]) == list:
        ...
        for part in bodystructure_header:
            if (
                ...
                ):  # bodystructure_header[2][1] filename position
                    # bodystructure_header[6] size
                    attachments += [
                        tuple(
                            (
                ...
                ):  # part[1][1] might be another filename position
                    attachments += [
                        tuple(
...
def get_all_emails(host, username, password):
    ...
    for folder in folders:
        ...
        for msg_id, data in fetched.items():
            ...
            sender_name, sender_email = get_sender_from_header(
                parsed_header.get("From", "")
            )

            list_unsubscribe_post = bool(
                parsed_header.get("List-Unsubscribe-Post", False)
            )
            ...
            email_headers = {
                ...
                "list_unsubscribe": get_list_unsubscribe(
                    parsed_header.get("List-Unsubscribe", ""), list_unsubscribe_post
                ),  # List-Unsubscribe
